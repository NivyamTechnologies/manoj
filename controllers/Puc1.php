...
where mlm_puc_detail.applicant_no = '".$this->session->userdata('applicant_no')."'
	...
	{
	$result = $this->db->query("SELECT (sum(totalwithtax)*20)/100 rpcomm from mlm_dist_chalan where billfromtype=9 and  createby = '".$this->session->userdata('empid')."' and MONTH(`datetime`) = MONTH(CURRENT_DATE())");
    return $result->result_array(); 	
	}
