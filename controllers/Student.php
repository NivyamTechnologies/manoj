...
*/
	...
	$this->load->view('student/menu_manage',$data);
	}
	...
	$data['data1']=$lll;
	$this->view('student/menu_manage_sidebar',$data);
	}
	...
	function index()
	{
		...
		redirect('student/dashboard',$this->data);
	}
	/***Admin Dashboard***/
	function dashboard()
	{
		...
		"SELECT * FROM tbl_student WHERE DATE_FORMAT((dob),'%m-%d') = DATE_FORMAT(NOW(),'%m-%d')");
		
		//Birthday List
		$birthdaycalendar 				= $this->base_model->run_query(
		"SELECT * FROM  tbl_student WHERE  DATE_ADD(dob, INTERVAL YEAR(CURDATE())-YEAR(dob) + IF(DAYOFYEAR(CURDATE()) > DAYOFYEAR(dob),1,0) YEAR) BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 MONTH) ORDER BY DATE_FORMAT(dob,'%m-%d')");
		...
		$this->data['content'] 				= 'student/dashboard';		 
		$this->view('student/dashboard',$this->data); 	
	}
	...
	function Accountinfo()
	{
		 if ($this->input->post()) {
				...
				$inputdata['residence_phone'] 			= $this->input->post('mobileno');
				...
				$where['Stdid'] 						= $this->session->userdata('empid');				
				//step for Update
				...
				return redirect('student/profile','refresh');
		 }
	...
	function profile()
	{
		...
		$viewstudentinfo = $this->base_model->run_query(
		"select * from tbl_student where Stdid ='".$this->session->userdata('empid')."'");
	  ...
	  $this->data['stateDrop']= $this->base_model->getStateByCountry();
	   $this->data['cityDrop']=$this->base_model->getStateByCity();
		if(isset($_POST['student_update'])!='')
		{
			...
			$this->form_validation->set_rules('residence_mobile', 'Mobile No.', 'required|regex_match[/^[0-9]{10}$/]');
			if ($this->form_validation->run() == true)
			{
				$data['Stdid'] 				= $this->input->post('registration_id');
				...
				$data['adharcard_no'] 		= $this->input->post('aadhar_cardno');
				$data['last_school_name'] 	= $this->input->post('lastschool_name');
				$data['last_class_passed'] 	= $this->input->post('lastclass_passed');
				$data['last_school_stard_date'] 		= $this->input->post('lastschool_startdate');
				$data['last_school_end_date'] 			= $this->input->post('lastschool_enddate');
				
				$data['last_class_persentage'] 		= $this->input->post('last_persentage');
				$data['last_school_board'] 			= $this->input->post('lastschool_board');
				...
				$data['residence_phone'] 	= $this->input->post('residence_mobile');
				$data['emergency_phone'] 	= $this->input->post('emergency_mobile');
				...
				$data['update_date'] 		= date('Y-m-d');
					...
					$this->session->set_flashdata('success','<font color="#05BD14">Student Information successfully Updated....</font>');
					return redirect('student/profile/',$this->data);
				}	
		}
			$this->data['title'] 		= 'Add Student';
			$this->data['active_menu'] 	= 'add student';
			$this->data['content'] 		= 'student/profile';
		 ...
		 if ($this->input->post()) {
			...
			//Upload User Photo
				...
				$where['Stdid'] 		= $this->session->userdata('empid');
					...
					'select student_image from '.$this->db->dbprefix('tbl_student')
					.' where Stdid ="'.$where['Stdid'].'"'
					);
					if (count($r) > 0) {
					
						if (file_exists('studentimage/'.$r[0]->student_image)) {
							unlink('studentimage/'.$r[0]->student_image);
						}						
					}
					...
					$img = $ext[0]."".$where['Stdid'].".".$ext[1];
					
					$inputdata['student_image'] = $img;
					move_uploaded_file(
					...
					//Set User Image
					$this->session->set_userdata('student_image',$img);
...
Copyright &copy; 2016 Onlinesoftservices Company. 14455 N. Hayden Rd, Ste. 219, Scottsdale, AZ 85260. All rights reserved.</font></td>
	...
	function attendance()
	{
		...
		{
		$month = 	$this->input->post('s1');
		$year = date('Y');
		$days =  date('m', strtotime($month));
		$d=cal_days_in_month(CAL_GREGORIAN,$days,$year);
		$attendance =  array();
		...
		"select * from tbl_student where Stdid ='".$this->session->userdata('empid')."' and status='Active'");
		$this->data['userinfo'] 	= $userinfo;
		$this->data['empid'] = $this->session->userdata('empid');
		$attendance_detail = $this->base_model->run_query(
		"select * from tbl_attendance where Stdid ='".$this->session->userdata('empid')."' and month ='".$month."'");
		$this->data['attendance_detail'] 	= $attendance_detail;
	$this->data['days']    =   $month;

		$this->data['title'] 				= 'Student Attendance';
		$this->data['active_menu'] 			= 'Attendance';
		$this->data['content'] 				= 'student/attendance';		
$part = 1;
$this->data['part'] 				= $part;

		$this->view('student/attendance', $this->data);
...
else
{
		...
		$logindt = date("Y/m/d",$cDate); //$cDate
		...
		{
		$studentattendance  = $this->base_model->run_query("SELECT * FROM tbl_attendance WHERE (date BETWEEN '".$this->input->post('from_date')."' AND '".$this->input->post('to_date')."') and Stdid = '".$this->session->userdata('empid')."' ORDER BY date LIMIT 31");
			$this->data['studentattendance'] 	= $studentattendance;
		...
		return redirect('student/attendance','refresh');
		}
	...
	function generate_excel()
	{
		...
		$this->data['allattendancelist'] = $this->base_model->ExportCSV();
		 $this->ExportCSV();
		 return redirect("student/attendance",$this->data);      
     }
	...
	{	
		...
		$Allclass = $this->base_model->run_query("select * from tbl_class");
		
		$this->data['Allclass'] 	= $Allclass;
		
		
			$Alltimetable = $this->base_model->run_query("select t.* from tbl_timetable as t,tbl_student as s where t.class_id=s.student_class and Stdid = '".$this->session->userdata('empid')."'");
			$this->data['Alltimetable'] 	= $Alltimetable;
			
		$this->data['title'] 		= 'timetable';
		$this->data['active_menu'] 	= 'timetable';
		$this->data['content'] 		= 'student/timetable';
		$this->view('student/timetable',$this->data); 
	}
	
	function fee()
	{
		
		$feeinfo = $this->base_model->run_query(
		"select * from tbl_student where Stdid ='".$this->session->userdata('empid')."'");
		
		$this->data['feeinfo'] 	= $feeinfo;
		$this->data['coaching'] = $this->base_model->getCoaching();
		
				
		$this->data['title'] 		= 'feedetails';
		$this->data['active_menu'] 	= 'feeinfo';
		$this->data['content'] 		= 'admin/feedetails';
		$this->view('student/feedetails',$this->data); 
	}
	
	//View All Subject & Teacher
	function teacherdetails()
	{
		
		$Allassignteacher = $this->base_model->run_query(
		"select a.class_id,a.subject_id,a.teacher_id from tbl_assignsubject as a,tbl_student as s where s.Stdid ='".$this->session->userdata('empid')."' and a.class_id=s.student_class");
		$this->data['class'] = $this->base_model->getClass();
		...
		$this->data['Allassignteacher'] 	= $Allassignteacher;		
		$this->data['title'] 		= 'teacherdetails';
		$this->data['active_menu'] 	= 'viewteacher';
		$this->data['content'] 		= 'admin/teacherdetails';
		$this->view('student/teacherdetails',$this->data); 
	}
...
<td colspan="2" style="font-size:12px; font-weight:bold; padding:0px 0px 0px 10px;">Leave OTP: '.$tokenno.'</td>
  ...
  <td colspan="2" style="font-size:10px; font-weight:bold; padding:0px 0px 0px 10px;">Online HRMS Team</td>
		...
		$this->data['content'] 					= 'student/leave';		
		$this->view('student/leave', $this->data);
	}
