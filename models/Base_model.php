  ...
  public function getBank()  
	...
	*/
	...
	function bvnew($apno,$mon,$year)
	{
		...
		$result2 = $this->db->query("SELECT sum(totalbv) as totalbv FROM `mlm_dist_chalan` WHERE branch_id in ($pp)   and YEAR(`datetime`) <= '".$year."'");
		$result3 = $this->db->query("SELECT sum(`bv`) as totalbv1 FROM `old_bv_dp` WHERE applicant_no in ($pp)  and YEAR(`date`) <= '".$year."'");
