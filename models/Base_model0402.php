  ...
  public function getBank()  
	...
	*/
	...
	function bvnew($apno,$mon,$year)
	{
		...
		$currentbv = $curbv[0]['monthbv']+$curbv1[0]['monthbv1'];
		
		$result2 = $this->db->query("SELECT sum(totalbv) as totalbv FROM `mlm_dist_chalan` WHERE branch_id in ($pp)  and MONTH(`datetime`) <= '".$mon."' and YEAR(`datetime`) <= '".$year."'");
		$result3 = $this->db->query("SELECT sum(`bv`) as totalbv1 FROM `old_bv_dp` WHERE applicant_no in ($pp) and MONTH(`date`) <= '".$mon."' and YEAR(`date`) <= '".$year."'");
		
		$curbv2=$result2->result_array();
		$curbv3=$result3->result_array();
		$totaltillnowbv = $curbv2[0]['totalbv']+$curbv3[0]['totalbv1'];
    // $totaltillnowbv = $curbv2[0]['totalbv']+$curbv3[0]['totalbv1']+$currentbv;
		
		return array($currentbv, $totaltillnowbv); 	
	}
	...
	*/
	...
	function getcomplainslno($type)
	{
	$query = $this->db->query("SELECT max(complain_id)+1 as serl_no from complain");	
	$curbv = $query->result_array();
	...
	{
	$query = $this->db->query("select (COALESCE(max(s_no),0)+1) as serl_no from (SELECT s_no FROM `mlm_puc_chalan` where billfromtype = '".$type."' and  createby = '".$this->session->userdata('empid')."'  and  date(datetime)>='2019-04-01'  union select s_no from mlm_dist_chalan where billfromtype = '".$type."' and  createby = '".$this->session->userdata('empid')."' and date(datetime)>='2019-04-01') as kk ");	
	$curbv = $query->result_array();
	return $curbv[0]['serl_no'];
	}
	
	function getserialnoviatypepucdistbrnach($type,$chalantype)
	{
	$query = $this->db->query("select (COALESCE(max(s_no),0)+1) as serl_no from (SELECT s_no FROM `mlm_puc_chalan` where  createby = '".$this->session->userdata('empid')."' and date(datetime)>='2019-04-01'  union select s_no from mlm_dist_chalan where createby = '".$this->session->userdata('empid')."' and date(datetime)>='2019-04-01'  union select s_no from mlm_scheme_chalan
	where createby = '".$this->session->userdata('empid')."' and date(datetime)>='2019-04-01') as kk ");	
	$curbv = $query->result_array();
	return $curbv[0]['serl_no'];
	}
	...
	{
	$query = $this->db->query("select sum(quan) as productsale from (SELECT sum(quantity) as quan FROM `mlm_puc_chalan` join mlm_puc_chalan_detail on mlm_puc_chalan.chalan_id = mlm_puc_chalan_detail.chalan_id where `product_id` = '".$productid."' and createby in('7','8460','8219')   union select sum(quantity) as quan from mlm_dist_chalan join mlm_dist_chalan_detail on mlm_dist_chalan.chalan_id=mlm_dist_chalan_detail.chalan_id where `product_id` = '".$productid."' and createby in('7','8460','8219')  and schemeid=0 union select sum(quantity) as quan from mlm_branch_chalan join mlm_branch_chalan_detail on mlm_branch_chalan.chalan_id=mlm_branch_chalan_detail.chalan_id where `product_id` = '".$productid."' and createby in('7','8460','8219')) as sale");	
	$curbv = $query->result_array();
	return $curbv[0]['productsale'];
	}

	function schme($productid)
	{
	$query = $this->db->query("SELECT sum(quantity) as quan FROM mlm_scheme_chalan where product_id ='".$productid."' and createby in('7','8460','8219')  and billfromtype = 2");	
	$curbv = $query->result_array();
	return $curbv[0]['quan'];
	}

	function schme1($productid)
	{
	$query = $this->db->query("select sum(quantity) as quan from mlm_dist_chalan join mlm_dist_chalan_detail on mlm_dist_chalan.chalan_id=mlm_dist_chalan_detail.chalan_id where product_id = '".$productid."' and  billfromtype = 2 and createby in('7','8460','8219')  and (datetime)>='2018-04-01' and product_total=0");	
	$curbv = $query->result_array();
	return $curbv[0]['quan'];
	}


function pucstock($id,$member_id){

	$query1 =$this->db->query("SELECT sum((pucproductquantity*product_dp)) stock FROM `puc_product_details`
left join tbl_product_inventory
on puc_product_details.productid = tbl_product_inventory.id
WHERE applicant_no='".$id."'");	
	$pucstock = $query1->result_array();
	return $pucstock[0][stock] ;
}
function puclimit($id,$member_id){
	$query2 =$this->db->query("SELECT creditlimit from mlm_puc_detail where applicant_no='".$id."'");	
	$limit = $query2->result_array();
	return $limit[0]['creditlimit'];
}
function pucrecive($id,$member_id){

	$query =$this->db->query("select sum(totalwithtax) total from mlm_puc_chalan where  receive=0 and  branch_id = '".$id."' and date(datetime)>='2019-04-01'");	
	$receive = $query->result_array();
	return $receive[0]['total'];
}


	function creditlimitpuc($id,$member_id){
...
WHERE applicant_no='".$id."'");	
	...
	$limit = $query2->result_array();
	 ...
	 $branchledger =   $this->base_model->pucnewledger($id,$member_id);

	 $pucrecive =   $this->base_model->pucrecive($id,$member_id);
		$totalvalue = $limit[0]['creditlimit']-($branchledger+ $pucstock[0][stock]+$pucrecive);
 return $totalvalue;
	}


function pucnewledger($id,$member_id){

	$query = $this->db->query("select (sum(totaldp) - sum(amount)) as totalledger from ( select s_no,branch_id,branch_name,createby,totalwithtax totaldp,0 amount,datetime from mlm_dist_chalan where createby='".$member_id."' 
	and datetime >= '2019-04-01'
				union all select  '' a,recid,name,narration,amount,0,date
				from mlm_transaction 
	where transtype='4' and byid= '".$member_id."' and date >='2019-04-01'
	union all select  '' a,recid,name,narration,0,amount,date
	from mlm_transaction 
	where transtype='2' and recid1='".$id."' and date >= '".$branchdd."' 
			...
			union all
				...
				from mlm_transaction where transtype='4' and recid = '".$id."' and date >= '2019-04-01'
	union all select  '' sno,'' a ,'Opening Balance' b ,'Opening Balance' f,ifnull(opening_balance,0) ,'' d ,'".$id."'  e
			from mlm_puc_detail where applicant_no= '".$id."')v1 order by datetime");	
			$branchledger = $query->result_array();

			return $branchledger[0]['totalledger'] ;


}


function branchstock($id,$member_id){

	$query1 =$this->db->query("SELECT sum((pucproductquantity*product_dp)) stock FROM `puc_product_details`
	left join tbl_product_inventory
	on puc_product_details.productid = tbl_product_inventory.id
	WHERE applicant_no='".$id."'");	
	$branchstock = $query1->result_array();
	return $branchstock[0][stock] ;
}
function branchlimit($id,$member_id){
	$query2 =$this->db->query("SELECT creditlimit from mlm_branch_detail where applicant_no='".$id."'");	
	$limit = $query2->result_array();
	return $limit[0]['creditlimit'];
}


function creditlimitbranch($id,$member_id){
...
WHERE applicant_no='".$id."'");	
	...
	$limit = $query2->result_array();
	 ...
	 $branchledger =   $this->base_model->branchnewledger($id,$member_id);
	
		$totalvalue = $limit[0]['creditlimit']-($branchledger+ $pucstock[0][stock]);
 return $totalvalue;
	}
...
function branchnewledger($id,$member_id){

	$query = $this->db->query("select  (sum(totaldp) - sum(amount)) as totalledger from ( select s_no,branch_id,branch_name,createby,totaldp,0 amount,datetime from mlm_dist_chalan where createby='".$member_id."' 
			and datetime >= '2019-04-01'
            union all select  '' a,recid,name,narration,amount,0,date
            from mlm_transaction 
			where transtype='4' and byid='".$member_id."' and date >= '2019-04-01' 
            ...
            select  '' a,recid,name,narration,0,amount,date 
            from mlm_transaction where transtype='4' and recid = '".$id."' and date >= '2019-04-01'
			union all select  '' sno,'' a ,'Opening Balance' b ,'Opening Balance' f,ifnull(Opening_balance_2019,0) ,'' d ,'2019-04-01'  e
	...
	{
	$query = $this->db->query("select sum(quan) as productsale from (SELECT sum(quantity) as quan FROM `mlm_puc_chalan` join mlm_puc_chalan_detail on mlm_puc_chalan.chalan_id = mlm_puc_chalan_detail.chalan_id where `product_id` = '".$productid."' and billfromtype = 2 and createby = '".$uid."' and (datetime)>='2019-04-01' union all select sum(quantity) as quan from mlm_dist_chalan join mlm_dist_chalan_detail on mlm_dist_chalan.chalan_id=mlm_dist_chalan_detail.chalan_id where `product_id` = '".$productid."' and  billfromtype in (2.,9) and createby = '".$uid."' and (datetime)>='2019-04-01' and schemeid=0) as sale");	
	$curbv = $query->result_array();
	return $curbv[0]['productsale'];
	}

	function schemedist($productid,$uid)
	{
	$query = $this->db->query("
	select sum(quan) as productsale from (SELECT sum(quantity) as quan FROM mlm_scheme_chalan 
	 where product_id = '".$productid."' and billfromtype = 2 and createby = '".$uid."' and (datetime)>='2019-02-01'
	union all
	select sum(quantity) as quan from 
	mlm_dist_chalan join mlm_dist_chalan_detail 
	on mlm_dist_chalan.chalan_id=mlm_dist_chalan_detail.chalan_id where product_id = '".$productid."'
	and  billfromtype = 2 and 
	createby = '".$uid."' and (datetime)>='2019-02-01' and product_total=0))as total");	
	$curbv = $query->result_array();
	return $curbv[0]['productsale'];
	}
	...
	function getpucbranchname($brno)
	{
	$alldata = $this->db->query("select a.member_id,a.applicant_no,a.email,a.password,a.role,a.status,b.bvmbranchname as brname,c.bvmbranchname as dpname,e.bvmbranchname as pucname,f.applicant_name as membername,i.party_name as party_name from mlm_members_login as a left join mlm_branch_detail as b on 
	a.applicant_no=b.applicant_no left join mlm_depo_detail as c on a.applicant_no = c.applicant_no left join mlm_puc_detail as e on a.applicant_no = e.applicant_no left join mlm_members_detail as f on a.applicant_no = f.applicant_no left join mlm_ledger_detail as i on a.applicant_no = i.applicant_no  where (a.role='branch' or a.role='depo' or a.role='puc' or a.role='member' or a.role='ledger') and (a.member_id='".$brno."' or a.applicant_no='".$brno."')")->result_array();
	if($alldata[0]['brname'])
	{
    $name = $alldata[0]['brname'];
	}
	elseif($alldata[0]['pucname'])
	{
    $name = $alldata[0]['pucname'];
	}
	elseif($alldata[0]['membername'])
	{
    $name = $alldata[0]['membername'];
	}
	elseif($alldata[0]['dpname'])
	{
    $name = $alldata[0]['dpname'];
	}
	elseif($alldata[0]['party_name'])
	{
    $name = $alldata[0]['party_name'];
	}
	else{
	$name = 'BVMADMIN';	
	}
