...
*/
  ...
  public function getBank()  
	...
	*/
	...
	{
	$query = $this->db->query("select (COALESCE(max(s_no),0)+1) as serl_no from (SELECT s_no FROM `mlm_puc_chalan` where billfromtype = '".$type."' and  createby = '".$this->session->userdata('empid')."'  and  date(datetime)>='2018-04-01'  union select s_no from mlm_dist_chalan where billfromtype = '".$type."' and  createby = '".$this->session->userdata('empid')."' and date(datetime)>='2018-04-01') as kk ");	
	$curbv = $query->result_array();
	...
	{
	$query = $this->db->query("select (COALESCE(max(s_no),0)+1) as serl_no from (SELECT s_no FROM `mlm_puc_chalan` where billfromtype = '".$type."' and chalantype = '".$chalantype."' and  createby = '".$this->session->userdata('empid')."' and date(datetime)>='2018-04-01'  union select s_no from mlm_dist_chalan where billfromtype = '".$type."' and chalantype = '".$chalantype."' and  createby = '".$this->session->userdata('empid')."' and date(datetime)>='2018-04-01') as kk ");	
	$curbv = $query->result_array();
	return $curbv[0]['serl_no'];
	}
	...
	{
	$query = $this->db->query("select sum(quan) as productsale from (SELECT sum(quantity) as quan FROM `mlm_puc_chalan` join mlm_puc_chalan_detail on mlm_puc_chalan.chalan_id = mlm_puc_chalan_detail.chalan_id where `product_id` = '".$productid."' and billfromtype = 1  union all select sum(quantity) as quan from mlm_dist_chalan join mlm_dist_chalan_detail on mlm_dist_chalan.chalan_id=mlm_dist_chalan_detail.chalan_id where `product_id` = '".$productid."' and billfromtype = 1 and schemeid=0) as sale");	
	$curbv = $query->result_array();
	...
	{
	$query = $this->db->query("select sum(quan) as productsale from (SELECT sum(quantity) as quan FROM `mlm_puc_chalan` join mlm_puc_chalan_detail on mlm_puc_chalan.chalan_id = mlm_puc_chalan_detail.chalan_id where `product_id` = '".$productid."' and billfromtype = 2 and createby = '".$uid."' and (datetime)>='2018-04-01' union all select sum(quantity) as quan from mlm_dist_chalan join mlm_dist_chalan_detail on mlm_dist_chalan.chalan_id=mlm_dist_chalan_detail.chalan_id where `product_id` = '".$productid."' and  billfromtype = 2 and createby = '".$uid."' and (datetime)>='2018-04-01' and schemeid=0) as sale");	
	$curbv = $query->result_array();
	return $curbv[0]['productsale'];
	}
	...
	->result_array();
	if(isset($alldata[0]['brname']))
	{
    $name = $alldata[0]['brname'];
	}
	elseif(isset($alldata[0]['pucname']))
	{
    $name = $alldata[0]['pucname'];
	}
	elseif(isset($alldata[0]['membername']))
	{
    $name = $alldata[0]['membername'];
	}
	elseif(isset($alldata[0]['dpname']))
	{
    $name = $alldata[0]['dpname'];
	}
	elseif(isset($alldata[0]['party_name']))
	{
