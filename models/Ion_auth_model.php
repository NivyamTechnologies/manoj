...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		{	
			...
			$img = $ext[0]."_".$id.".".$ext[1];
			move_uploaded_file($_FILES['image']['tmp_name'], 'assets/uploads/images/'.$img);
			$myControllerObj = new MY_Controller;
			$myControllerObj->create_thumbnail('assets/uploads/images/'. $img, 'assets/uploads/images(200x200)/'. $img,200,200);
			$myControllerObj->create_thumbnail('assets/uploads/images/'. $img, 'assets/uploads/images(50x50)/'. $img,50,50);

			$updatedata['image'] = $img;
			$this->db->update($this->tables['users'], $updatedata, array('id' => $id));	
	...
	$query = $this->db->select($this->identity_column . ', username, email, id, password, image, active, last_login')
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				//$this->session->set_userdata('sess_expiration',1209600); //2 weeks
	...
	public function update($id, array $data)
	{
		...
		{	
			$image = $data['image'];
			$ext = explode('.', $_FILES['image']['name'] );
			move_uploaded_file($_FILES['image']['tmp_name'], 'assets/uploads/images/'.$image);
			$myControllerObj = new MY_Controller;
			$myControllerObj->create_thumbnail('assets/uploads/images/'. $image, 'assets/uploads/images(200x200)/'. $image,200,200);
			$myControllerObj->create_thumbnail('assets/uploads/images/'. $image, 'assets/uploads/images(50x50)/'. $image,50,50);			
		}
