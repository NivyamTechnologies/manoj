						 ...
						 <td style="padding-right: 25px;">From Date</td> 
						  ...
						  <td style="padding-right: 25px;"><input type="submit" class="btn btn-primary" value="search" name="search"/></td>
							...
							<tr>
								...
								<th>
							    No.Of Quantity(Opening Balance)
								</th>
								<th>
							...
							<tr>								
                               ...
                               <td>
								...
								<td>
								<?php
								echo $sale = $this->base_model->productsalepuc($r->id,$uid)	;
								?>
								</td>
								<td>
								<?php echo $bal = ($r->total_quantity + $r->receive - $sale);
								$bal_total = $bal_total+$bal;
								$this->db->query("update puc_product_details set pucproductquantity = '".$bal."' where productid='".$r->id."' and  applicant_no='".$this->session->userdata('applicant_no')."'");
								...
								$proinfo_total = $proinfo_total+$proinfo[0]['product_mrp_price']*$bal; 
								echo $proinfo[0]['product_mrp_price']*$bal;  	
								$this->db->query("update pucstock set totalvalue = '".$proinfo_total."' where  applicant_no='".$this->session->userdata('applicant_no')."'");
